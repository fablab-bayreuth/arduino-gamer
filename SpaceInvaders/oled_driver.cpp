...
OledDriver::OledDriver(int mosi_pin, int clk_pin, int dc_pin, int cs_pin, int rst_pin) {
...
void OledDriver::init() {
  ...
  //SM: Add hardware reset sequence
...
void OledDriver::writeCharacter(const char character) {
  for(int i=0; i<FONT_WIDTH; i++) writeData(FONT[character - 0x20][i]);
}

void OledDriver::writeString(const char *characters) {
