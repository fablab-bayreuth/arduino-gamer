...
void Arduboy2Core::bootPins()
{
  ...
  // Speaker: Not set here. Controlled by audio class
    ...
    // SM: FLB Gamer Clone ports init
  PORTB &= ~( _BV(SPI_MOSI_BIT) | _BV(SPI_SCK_BIT) |        // SPI and display control lines lines low
              _BV(CS_BIT) | _BV(DC_BIT) | _BV(RST_BIT) );
  DDRB |= _BV(SPI_MOSI_BIT) | _BV(SPI_SCK_BIT) |            //   and as output
          _BV(CS_BIT) | _BV(DC_BIT) | _BV(RST_BIT);;          
  // Note: On a production system, unused pins should be configured as outputs,
  //   but we do not care at the moment.
#endif
}
...
// master, mode 0, MSB first, CPU clock / 2 (8MHz)
  ...
  // SM: Note: See ATmega328 datasheet sect. 32.3.2: 
  //  "If SS is configured as an input, it must be held high to ensure Master SPI operation.""
  // On the Gamer hardware, SS (PB2) is used as Display DC, and is configured as output.
}
...
void Arduboy2Core::bootPowerSaving()
{
  ...
  #else
    // On the gamer, keep adc on (needed for ananlog stick), only disable I2C
    power_twi_disable();  // Device independent macro from avr/power.h
  #endif
